  ...
  def reconcile do
    ...
    mismatch_count =
      Enum.reduce(agent_ids, 0, fn agent_id, acc ->
        if State.has_pending_settlement_for_observer?(agent_id) do
          Logger.info(
            "[reconcile] defer agent #{agent_id}: #{State.pending_event_count(agent_id)} pending settlement event(s)"
          )
          ...
          if on_chain != local do
            Logger.warning(
              "Reconciliation mismatch: agent #{agent_id}, chain=#{on_chain}, local=#{local}"
            )
            ...
            Phoenix.PubSub.broadcast(
              AFW.PubSub,
              "guardian",
              {:reconciliation_mismatch, agent_id, on_chain, local}
            )
    ...
    if mismatch_count == 0 do
      ...
      Logger.warning(
        "[reconcile] check #{length(agent_ids)} agents, #{mismatch_count} mismatches"
      )
    end
