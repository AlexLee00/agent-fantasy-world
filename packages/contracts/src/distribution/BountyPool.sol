...
contract BountyPool is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    {
        require(recipients.length == amounts.length, "BountyPool: length mismatch");

        uint256 totalAmount;
        for (uint256 i = 0; i < recipients.length; i++) {
            require(recipients[i] != address(0), "BountyPool: recipient required");
            require(amounts[i] > 0, "BountyPool: amount required");
